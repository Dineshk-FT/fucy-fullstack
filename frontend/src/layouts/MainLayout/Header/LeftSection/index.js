...
const LeftSection = () => {
  ...
  const handleGenerateTestCases = async (e) => {
    ...
    // Provide payload (You can later pull exact node details instead of these placeholders)
    const payload = {
      modelId: model._id,
      asset: assets?.name || 'Gateway ECU',
      interface: 'UDS',
      threat: 'Unauthorized Diagnostic Access',
      attackPath: 'CAN -> UDS -> Security Access',
      risk: 'High'
    };
    ...
    if (res && !res.error) {
      ...
      notify(res.message || 'Test cases generated successfully', 'success');
    } else {
