...
const RenderedTreeItems = ({
  ...
  const handlePropertiesTab = useCallback((detail, type) => {
  ...
  const renderProperties = useCallback((properties, detail, type) => {
  ...
  const renderSubItems = useCallback((subs, handleOpenTable, contextMenuHandler, additionalMapping) => {
    return subs?.map((sub) =>
  ...
  const renderSection = useCallback((nodeId, label, details, type) => {
