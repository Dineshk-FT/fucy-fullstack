  ...
  useEffect(() => {
    const fetchCatalogData = async () => {
      try {
        ...
        const extractedRows = Object.entries(catalogData.unintendedHumanActionsThreats.threats).flatMap(
          ([threatKey, threatValue], threatIndex) =>
            Object.entries(threatValue.sub_threats).map(([subThreatKey, subThreatValue], subIndex) => ({
              id: catalogData.unintendedHumanActionsThreats.ID,
              name: subIndex === 0 ? catalogData.unintendedHumanActionsThreats.description : '',
...
export default HumanActionTable;