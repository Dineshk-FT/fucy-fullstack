...
const ExternalConnectivityTable = () => {
  ...
  useEffect(() => {
    const fetchCatalogData = async () => {
      try {
        ...
        const extractedRows = Object.entries(catalogData.externalConnectivityThreats.threats).flatMap(
          ([threatKey, threatValue], threatIndex) =>
            Object.entries(threatValue.sub_threats).map(([subThreatKey, subThreatValue], subIndex) => ({
              id: catalogData.externalConnectivityThreats.ID,
              name: subIndex === 0 ? catalogData.externalConnectivityThreats.description : '',
    ...
    >
      <Box display="flex" justifyContent="space-between" alignItems="center" my={1} mx={1}>
        <Typography sx={{ color: color?.title, fontWeight: 600, fontSize: '16px' }}>
          Vehicles from External Connectivity and Network Connections Table
        </Typography>
        <TextField
          placeholder="Search"
          size="small"
...
export default ExternalConnectivityTable;